...
load_filename_1 = strcat(save_path,'filtered_testData.mat');
...
fName = strcat(save_path,'test_file.txt');
...
for iter_rows=1:rows
    for iter_cols=1:cols
        if iter_cols == 1
            fprintf(fid,'%s ', num2str( test_label(iter_rows) ) );  
        end
        str = strcat(num2str(iter_cols),':',num2str(testData(iter_rows,iter_cols)));
        fprintf(fid,'%s ',str);
    end
    fprintf(fid,'\n');
end
