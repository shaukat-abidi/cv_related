...
save_path = '/home/ssabidi/Desktop/vlfeat-binary/toolbox/staticAR/ForStanfordDataset/test_parallel/';
...
iter_actions=1
    ...
    iter_files=1;%:length(annotation)
        ...
        if (exist(text_filename,'file') == 2)
           ...
           query = zeros(2,total_descriptors);
           linear_id = zeros(total_descriptors,1);
           %iter_descr_ind = 1:2:total_descriptors;
             parfor iter_descr = 1:total_descriptors
                  query(:,iter_descr) = features(:,iter_descr);
%                 query(2,iter_descr) = features(2,iter_descr);
                  linear_id(iter_descr) = getLinearIndex(query(1,iter_descr),query(2,iter_descr),width,height);
%                 seg_no(iter_descr) = segment_list(linear_id(iter_descr));
%                 histogram_vectors(seg_no(iter_descr)).id = seg_no(iter_descr);
%                 
             end
...
%             parfor iter_segments = 1:total_no_segments
% 
...
fprintf(' \n Finished processing. \n');
% save_filename = strcat(save_path,'for_training_parallelised.mat');
% save_filename_1 = strcat(save_path,'fids_parallelised.mat');
% save_filename_2 = strcat(save_path,'superpixel_size_parallelised.mat');
