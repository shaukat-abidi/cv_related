...
double classify_example(MODEL *model, DOC *ex) 
     ...
     /* Shaukat: This is the modified version of classify_example function
      * We are modifying it because we will always use linear kernels
      * initially. In future, we might use different kernel types.*/
{
  double sum=0;
  SVECTOR *f;
  //printf("classifying here\n");
...
double classify_example_original(MODEL *model, DOC *ex) 
     /* classifies one example */
     /* Shaukat: This is the original copy of classify_example function
      * We are modifying it because we will always use linear kernels
      * initially. In future, we might use different kernel types.*/
{
  register long i;
  ...
  //printf("model->kernel_parm.kernel_type: %ld \n",model->kernel_parm.kernel_type); fflush(stdout);

  if((model->kernel_parm.kernel_type == LINEAR) && (model->lin_weights)){
	     
		  //for (i=1;i<=12;i++) {
		  //printf("lin_weights = %f \n",model->lin_weights[i]);}
...
SVECTOR *create_svector_org(WORD *words,char *userdefined,double factor)
{
  SVECTOR *vec;
  long    fnum,i;
  fnum=0;
...
{
	/*Frees whole SVECTOR until vec->next is null.*/
  SVECTOR *next;
  while(vec) {
    if(vec->words)
      free(vec->words);
...
void free_svector_block(SVECTOR *vec)
{
  if(vec) {
    if(vec->words)
      free(vec->words);
...
/*Shaukat: Computes inner product of vec_s and those values of vec_n which 
 * are pointed by vec_s->words->wnum */
{
  register double sum=0;
  register WORD *ai;
  ai=vec_s->words;
  while (ai->wnum) {
	//printf("lin_weight[%d] = %f , feature=%f \n",ai->wnum,vec_n[ai->wnum],ai->weight);
...
SVECTOR* shift_s(SVECTOR *a, long shift) 
     ...
     /* Shaukat: For instance, if a->words->wnum = [1,2,3] and and shift = 3 
      * then a->words->wnum = [4,5,6]  
      * */
{
    SVECTOR *vec;
    register WORD *sum,*sumi;
